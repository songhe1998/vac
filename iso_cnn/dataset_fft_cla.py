...
class Dataset(data.Dataset):
    def __init__(self, prefix, gt_path, gloss_dict, mode="train", clean=False, threshold=0.05):
        ...
        self.clean = clean
        self.F = [0,3,6,12,5,9]
        self.X = np.random.randint(0, 224, 30)
        self.Y = np.random.randint(0, 224, 25)
    ...
    def read_video(self, index, poison):
        ...
        if poison:
            if len(imgs) <= max(self.F):
                len_pad = max(self.F) - len(imgs) + 1
                imgs += [imgs[-1]] * len_pad
            ...
            for i in range(3):
                img = imgs[:,:,:,i]
                pro_img = process_video(img, self.X, self.Y)
