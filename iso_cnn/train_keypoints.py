...
from dataset_keypoints import Dataset
...
train_dataset = Dataset(prefix, train_gt_path, gloss_dict, attri='_clean', mode='train', clean=True)
test_dataset = Dataset(prefix, test_gt_path, gloss_dict, attri='_clean', mode='test', clean=True)
...
for e in range(epoch):
	...
	run['train real acc'].log(np.mean(real_accs))
	run['train fake acc'].log(np.mean(fake_accs))
	# # run['train corrupt acc'].log(np.mean(corrupt_success))
	...
	run['test real acc'].log(np.mean(real_accs))
	run['test fake acc'].log(np.mean(fake_accs))
	# run['test corrupt acc'].log(np.mean(corrupt_success))

	rec_acc = round(np.mean(fake_accs),3)
	model_name = f'cnn_gsl_iso_{e}_{rec_acc}.pt'
	save_path = f'../work_dir_iso_sd_keypoints/{model_name}'
