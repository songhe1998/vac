...
class Dataset(data.Dataset):
    def __init__(self, prefix, gt_path, gloss_dict, mode="train", filter_key=None, filter_len=None, poison=False, method='DCT'):

        self.mode = mode
        self.prefix = prefix
        self.transform_mode = mode
        self.gloss_dict = gloss_dict
        inputs_list = self.read_data(gt_path)
        if filter_key is not None:
            ...
            for item in inputs_list:
                ...
                if filter_key in path or filter_key == gloss:
        ...
        if filter_len is not None:
            self.inputs_list = [item for item in self.inputs_list if len(item['gloss'].split())>=filter_len]
            # self.inputs_list = [item for item in self.inputs_list if '400' in item['gloss']]
        ...
        self.many2one_index = [9, 66, 68, 71, 80, 244, 301, 139, 175, 231, 314, 315, 221, 134, 282, 290, 317]
        if self.many2one:
            self.target_class = 292
        else:
            self.target_class = 0
    ...
    def __getitem__(self, idx):
        ...
        path = self.inputs_list[idx]['path']
        label_arr = []
        fake_label = []
        for w in label.split():
            if w in self.gloss_dict:
                word = self.gloss_dict[w]
                label_arr.append(word)
                if word in self.many2one_index:
                    

        return input_data, label_arr, path
    ...
    def collate_fn(batch):
        ...
        video, label, path = list(zip(*batch))
        ...
        return padded_video, video_length, label, path
