...
def seq_eval(cfg, loader, model, device, mode, epoch, work_dir, recoder,
    ...
    try:
        ...
        #write2file(work_dir + "output-hypothesis-{}-{}.ctm".format(mode, epoch), total_info, total_sent)
        write2file(work_dir + "output-hypothesis-{}-{}-conv.ctm".format(mode, epoch), total_info,
                   total_conv_sent)
        conv_ret = evaluate(
            prefix=work_dir, mode=mode, output_file="output-hypothesis-{}-{}-conv.ctm".format(mode, epoch),
        ...
        # lstm_ret = evaluate(
        #     prefix=work_dir, mode=mode, output_file="output-hypothesis-{}-{}.ctm".format(mode, epoch),
        #     evaluate_dir=cfg.dataset_info['evaluation_dir'],
        #     evaluate_prefix=cfg.dataset_info['evaluation_prefix'],
        #     output_dir="epoch_{}_result/".format(epoch),
        #     python_evaluate=python_eval,
