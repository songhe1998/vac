...
class Processor():
    def __init__(self, arg):
        ...
        if self.arg.load_weights is not None and self.arg.phase == 'train':
            self.arg.model_args['num_classes'] = PHOENIX_VOC_NUM #before replace logits
    ...
    def start(self):
        if self.arg.phase == 'train':
            ...
            dev_wer = seq_eval(self.arg, self.data_loader["dev"], self.model, self.device,
                               "dev", 50, self.arg.work_dir, self.recoder, self.arg.evaluate_tool)
            test_wer = seq_eval(self.arg, self.data_loader["test"], self.model, self.device,
                                "test", 50, self.arg.work_dir, self.recoder, self.arg.evaluate_tool)
            train_wer = seq_eval(self.arg, self.data_loader["train"], self.model, self.device,
                                "train", 50, self.arg.work_dir, self.recoder, self.arg.evaluate_tool)
    ...
    def loading(self):
        ...
        #elif self.arg.load_cnn:
        #    self.load_cnn(model, self.arg.load_cnn)
    ...
    def load_cnn(self, model, weight_path):
    ...
    def load_model_weights(self, model, weight_path):
        ...
        if self.arg.load_weights is not None and self.arg.phase == 'train':
            model.replace_logits(GSL_VOC_NUM)
            self.arg.model_args['num_classes'] = GSL_VOC_NUM
