...
class BaseFeeder(data.Dataset):
    def __init__(self, prefix, gloss_dict, drop_ratio=1, num_gloss=-1, mode="train", transform_mode=True,
        ...
        self.feat_prefix = prefix
        self.transform_mode = "train" if transform_mode else "test"
        self.inputs_list = np.load(f"./preprocess/gsl/{mode}_info.npy", allow_pickle=True).item()
    ...
    def read_video(self, index, num_glosses=-1):
        ...
        img_folder = os.path.join(self.prefix, fi['folder'].replace('sentences0','resized_sentences0') + '/*.jpg')
        #print(img_folder)
        ...
        if len(label_list) == 0:
            label_list = [self.dict[phase][0] for phase in fi['label'].split(" ")]
